(function() {
    ...
    function readClassFile(dataBuffer) {
        ...
        if((cf.access_flags & ACC_INTERFACE) == ACC_INTERFACE) {
            ...
            if((cf.access_flags & ACC_FINAL) == ACC_FINAL
                || (cf.access_flags & ACC_SUPER) == ACC_SUPER
                || (cf.access_flags & ACC_ENUM) == ACC_ENUM) {
            ...
            if((cf.access_flags & ACC_ANNOTATION) == ACC_ANNOTATION) {
            ...
            if((cf.access_flags & ACC_FINAL) == ACC_FINAL && (cf.access_flags & ACC_ABSTRACT) == ACC_ABSTRACT) {
    ...
    function getConstPoolUtf8(bytesArray) {
        ...
        for(var i = 0, charLength; i < bytesArray.length; i+= charLength) {
            ...
            if((i + charLength - 1) >= bytesArray.length) {
                throw new FormatError("Unrecognisable modified-UTF8 sequence.");
            }
