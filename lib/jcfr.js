(function () {
    ...
    module.exports.readClassFile = function (filePath) {
        var fs = require("fs"),
            dataBuffer = fs.readFileSync(filePath),
            nativeBuffer = utils.bufferToArrayBuffer(dataBuffer);

        return this.readArrayBuffer(nativeBuffer);
    };

    module.exports.readArrayBuffer = function (dataBuffer) {
        if (!(dataBuffer instanceof ArrayBuffer)) {
            throw new TypeError("Buffer must be of type 'ArrayBuffer'.");
        }

        try {
            return readClassFile(dataBuffer);
        } catch (e) {
    ...
    function readClassFile(dataBuffer) {
        ...
        if (utils.isFlagSet(cf.access_flags, AccessFlag.ACC_INTERFACE)) {
            ...
            if (utils.isFlagSet(cf.access_flags, AccessFlag.ACC_FINAL)
                ...
                throw new FormatError("Invalid access_flags: ACC_FINAL, ACC_SUPER, ACC_ENUM cannot be set if ACC_INTERFACE is set.");
            }
        } else {
            ...
            if (utils.isFlagSet(cf.access_flags, AccessFlag.ACC_FINAL) && utils.isFlagSet(cf.access_flags, AccessFlag.ACC_ABSTRACT)) {
        ...
        if (cf.this_class >= (cf.constant_pool_count - 1) || cf.constant_pool[cf.this_class - 1].tag !== ConstPoolTag.CLASS_INFO) {
        ...
        if (cf.super_class !== 0) {
            ...
            if (cf.super_class >= (cf.constant_pool_count - 1) || cf.constant_pool[cf.super_class - 1].tag !== ConstPoolTag.CLASS_INFO) {
        ...
        cf.fields = readFields(cf, reader, cf.fields_count);

        cf.methods_count = reader.nextUInt16BE();
        cf.methods = readMethods(cf, reader, cf.methods_count);

        cf.attributes_count = reader.nextUInt16BE();
        cf.attributes = readAttributes(cf, reader, cf.attributes_count);
        ...
        for (var idx = 1; idx <= (constPoolCount - 1); idx++) {
            ...
            switch (cp_info.tag) {
                ...
                case ConstPoolTag.DOUBLE:
                    ...
                    // To keep our constant pool array consistent with the class file indexing, we will add another const
                    // pool entry, with the correct tag but no high_bytes or low_bytes properties.
    ...
    function readFields(cf, reader, fieldsCount) {
        var fields = [];
        ...
        for (var i = 0; i < fieldsCount; i++) {
            ...
            if (!(utils.isFlagSet(flag, AccessFlag.ACC_PRIVATE)
                ...
                throw new FormatError("Field info access flag can have at most one of ACC_PRIVATE, ACC_PROTECTED, and ACC_PUBLIC flags set.");
            }
            ...
            if (field_info.name_index >= (cf.constant_pool_count - 1)
                ...
                throw new FormatError("Field info properties name_index and descriptor_index must point to CONSTANT_Utf8_info structure.");
            }
            ...
            field_info.attributes = readAttributes(cf, reader, field_info.attributes_count);
    ...
    function readMethods(cf, reader, methodsCount) {
        var methods = [];
        ...
        for (var i = 0; i < methodsCount; i++) {
            ...
            method_info.attributes = readAttributes(cf, reader, method_info.attributes_count);
    ...
    function readAttributes(cf, reader, attributesCount) {
        var attributes = [];
        ...
        for (var i = 0; i < attributesCount; i++) {
            ...
            if (attribute_info.attribute_name_index >= (cf.constant_pool_count - 1)
                ...
                throw new FormatError("Attribute info property attribute_name_index must point to CONSTANT_Utf8_info structure.");
            }
